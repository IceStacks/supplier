...
namespace WebApi.Application.SupplierOperations.Validators
{
    public class UpdateSupplierCommandValidator : AbstractValidator<UpdateSupplierCommand>
    {
        public UpdateSupplierCommandValidator()
        {
            ...
            RuleFor(x => x.Model.Mail).EmailAddress().MaximumLength(30).When(y => y.Model.Mail.Trim() != string.Empty);
            RuleFor(x => x.Model.Phone).Length(15).When(y => y.Model.Phone.Trim() != string.Empty); // kontrol edilecek
            RuleFor(x => x.Model.CompanyName).MinimumLength(2).MaximumLength(20).When(y => y.Model.CompanyName.Trim() != string.Empty); 
            RuleFor(x => x.Model.CompanyMail).EmailAddress().MaximumLength(30).When(y => y.Model.CompanyMail.Trim() != string.Empty);
